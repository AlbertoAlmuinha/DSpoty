...
#' @examples
#' \dontrun{
#' get_artists('Arctic Monkeys')
#' }

get_artists<-function(artist_name, offset='0', limit='1', access_token = DSpoty::get_spotify_access_token()){
  ...
  artista<-map_df(seq_len(length(res)), function(this_row){
                ...
                list(
                  ...
                  artista_img = if(length(artist$images)>0){artist$images[[1]]$url} else {NA},
                  ...
                  estilo_musical = if(length(artist$genres)>0){str_replace_all(str_c(unlist(artist$genres), collapse = "-")," ","_")} else {NA}
