...
get_artists_albums<-function(artist_name, album_type = 'album',limit=50, offset=0,
  ...
  artist_uri<-DSpoty::get_artists(artist_name = artist_name, limit = 1) %>% .$artista_uri
  ...
  album<-map_df(seq_len(length(res)), function(this_row){
              ...
              list(
                ...
                tipo_album = if_else(length(album_info$album_type)>0,album_info$album_type, NULL),
                ...
                precision_fecha_lanzamiento = album_info$release_date_precision,
                total_canciones_album = album_info$total_tracks,
                mercados_disponibles = album_info$available_markets[[1]],
                popularidad = get_album_popularity(album_info$id)

              )
            }) %>% filter(!duplicated(tolower(album_uri))) %>%
                    filter(!duplicated(tolower(nombre_album)))
