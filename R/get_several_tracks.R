...
get_several_tracks<-function(tracks_id_df, ids_label = 1, access_token = DSpoty::get_spotify_access_token()){
  ...
  tracks<-map_df(seq_len(length(res)), function(this_row){
    ...
    list(
      ...
      cancion_uri = track$id,
      cancion_url = track$external_url$spotify,
      artista_uri = track$artists[[1]]$id,
      nombre_artista = track$artists[[1]]$name,
      cancion_mp3_60s = if_else(is.null(track$preview_url), "NULL", track$preview_url)
