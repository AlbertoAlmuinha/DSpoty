...
get_several_tracks_features_vectorized<-function(tracks_df, access_token = DSpoty::get_spotify_access_token()){
  ...
  tracks_features<-matrix(nrow = length(indices[[1]]), ncol = 13) %>% as.data.frame() %>% set_names(., nm = "CANCION_URI", "BAILABILIDAD", "ENERGIA", "TONO", "VOLUMEN", "MODO",
                                                                                                    "DISCURSOS", "ACUSTICA", "INSTRUMENTALIDAD", "VITALIDAD", "VALENCIA", "TEMPO",
                                                                                                    "COMPAS")
